...
if (client.users.has(req.user.id) === true) { // Hey ! @! ๖̶̶̶ۣۣۜۜ͜ζ͜͡xChairs botunuz olan "Radyo Bot" siraya eklendi.
  client.users.get(req.user.id).send(`Merhaba <@${req.user.id}>  Botunuz \`${sistem.username}\` başarıyla Best Discord Bots'a eklendi. \nhttps://bestdiscordbots.xyz/view/${ID}`)
}                                                                          
...
app.post("/sertifika/:botID", checkAuth, (req, res) => {
...
res.redirect("/yetkili")
  ...
  db.set(`sertifikak.${owneridd}.durum`, true)
    ...
    client.channels.get(client.ayarlar.kayıt).send(`<@${db.fetch(`botlar.${id}.sahipid`)}> adlı kullanıcının ${db.fetch(`botlar.${id}.isim`)} adlı botun sertifikası 0naylandı. `)
});
...
app.get("/botyonetici/sertifikareddet/:botID", checkAuth, (req, res) => {
 ...
 if(!db.has(`yetkili.${req.user.id}.durum`) ) return res.redirect('/yetkili/hata')
  ...
  res.redirect("/yetkili")
      ...
      client.channels.get(client.ayarlar.kayıt).send(`verification not approved ${db.fetch(`botlar.${id}.isim`)} by <@${db.fetch(`botlar.${id}.sahipid`)}> https://thediscordbots.xyz/view/${id}`)
...
client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`Merhaba, <@${db.fetch(`botlar.${id}.sahipid`)}> ${db.fetch(`botlar.${id}.isim`)} adlı botunuz ekibimiz tarafından onaylanmıştır.`)
 ...
 if(!db.has(`yetkili.${req.user.id}.durum`) ) return res.redirect('/yetkili/hata')
  ...
  res.redirect("/yetkili")
   ...
   client.channels.get("617809273497583617").send("<:red:617608486381158416> | <@" + req.body['red-sahipid'] + "> adlı kullanıcının **" + req.body['red-id'] + "** adlı botu **" + req.body['red-sebep'] + "** sebebi ile reddedildi.");
if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
client.users.get(db.fetch(`botlar.${id}.sahipid`)).send("<:red:617608486381158416> | <@ **" + req.body['red-id'] + "** adlı botunuz **" + req.body['red-sebep'] + "** sebebi ile reddedildi.");
}
  });
